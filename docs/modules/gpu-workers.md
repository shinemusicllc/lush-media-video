...
- The VPS stale-listener watchdog must defer `TERM` while `ss` reports an
  established forwarded connection on that reverse port for a bounded grace
  window. Slow uploads can block `/system_stats`; after the grace window,
  continued health failure is treated as a stale tunnel so one dead session
  cannot keep a worker offline forever.
