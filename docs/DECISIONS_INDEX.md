...
| GPU-004 | GPU1 runs from a visible interactive Desktop launcher instead of an active `SYSTEM` Scheduled Task. | Startup occurs after Windows login; the same singleton supervisor/watchdog runs in a visible console and the legacy task remains disabled for rollback. |
