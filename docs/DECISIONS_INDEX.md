...
| AUTH-001 | Keep JWT auth and add role-based account settings. | Admin manages accounts and selects `admin`/`user`; every role can create video, and at least one admin must remain. |
| AUTH-002 | All authenticated roles share one web job workspace. | Job creator identity is retained for audit, while visibility and terminal job actions are workspace-wide; admin-only boundaries remain account management and server administration. |
...
| DRIVE-001 | Google Drive image bytes are fetched by the selected ComfyUI worker. | A worker-side custom node loads shared Drive images directly; the backend persists the file ID for queue recovery and serves the Drive thumbnail without proxying the full input image. Both workers must install the node. |
| DEPLOY-004 | Backup retention invokes Python explicitly. | The timer must not depend on `cleanup_data.py` execute permissions; cleanup precedes archive creation and retains seven days by default. |
