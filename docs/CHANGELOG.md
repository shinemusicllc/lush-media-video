...
- Fixed: runtime data/secrets/backups are ignored by Git and Docker build context; the backup systemd service no longer runs as `deploy`, which could not read root-owned upload files.
