...
| Shared web job workspace across roles | All authenticated accounts must see and operate on the same admin workspace; retain the creator identity for audit without splitting job visibility by account. | Auth + queue UX | 2026-09-11 |
...
| Invoke retention cleanup via Python interpreter and promote archive atomically | `cleanup_data.py` lost its execute bit on the VPS, causing the daily backup timer to fail before it could prune expired uploads or archives. The wrapper now calls `PYTHON_BIN` explicitly, preserves the default seven-day policy, prunes before archive creation, and writes through a verified `.partial` archive so live uploads cannot leave a failed file presented as a backup. | Storage operations | 2026-09-18 |
